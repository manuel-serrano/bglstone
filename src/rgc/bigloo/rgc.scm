;*=====================================================================*/
;*    serrano/prgm/project/bglstone/src/rgc/bigloo/rgc.scm             */
;*    -------------------------------------------------------------    */
;*    Author      :  Manuel Serrano                                    */
;*    Creation    :  Tue Jun 30 13:36:49 1998                          */
;*    Last change :  Wed Oct 15 11:51:18 2014 (serrano)                */
;*    -------------------------------------------------------------    */
...
;*---------------------------------------------------------------------*/
(define-expander $create-vector
   (lambda (x e)
      (if (string<? (bigloo-config 'release-number) "3.4b")
	  `(c-create-vector ,@(map (lambda (x) (e x e)) (cdr x)))
	  `($create-vector ,@(map (lambda (x) (e x e)) (cdr x))))))
(define-expander $vector-ref
   (lambda (x e)
      (if (string<? (bigloo-config 'release-number) "3.4b")
	  `(c-vector-ref ,@(map (lambda (x) (e x e)) (cdr x)))
	  `($vector-ref ,@(map (lambda (x) (e x e)) (cdr x))))))
(define-expander $vector-set!
   (lambda (x e)
      (if (string<? (bigloo-config 'release-number) "3.4b")
	  `(c-vector-set! ,@(map (lambda (x) (e x e)) (cdr x)))
	  `($vector-set! ,@(map (lambda (x) (e x e)) (cdr x))))))
...
(define *grammar*
   (regular-grammar ((chiffre (in ("09")))
	  ...
	  (if (and (vector? *list-errors*)
	      ...
	      ($vector-set! *list-errors* open-key pos))
	  ...
	  (if (and (vector? *vector-errors*)
		 ...
		 ($vector-set! *vector-errors* open-key pos)))
	  ;; and then, we compute the result list...
	  (set! *par-open* (+fx 1 *par-open*))
		 ...
		 ((>fx *bra-open* 0)
		  (let ((open-key (-fx *bra-open* 1)))
		     ...
		     (if (and (<fx open-key (vector-length *vector-errors*))
			      (fixnum? ($vector-ref *vector-errors*
						     open-key)))
			 (error/location "read"
					 "Unclosed vector or structure"
					 char
					 (input-port-name (the-port))
					 ($vector-ref *vector-errors*
						       open-key))
			 (error "read"
				"Unexpected end-of-file"
				"Unclosed vector or structure"))))
