  ...
  (let ((v (make-vector (*fx nrows ncols))))
	...
	((<fx r 0))
      (do ((c 0 (+fx c 1))
	   (i (*fx r ncols) (+fx i 1)))
	  ((=fx c ncols))
	(vector-set! v i (proc (*fx 3 c) (+fx (*fx 2 r) (bitand c 1))))))
...
(define (make-wall-vec harr)
  (let* ((nrows (harr~nrows harr))
	...
	((<fx x 0))
      (do ((y (+fx (*fx (-fx nrows 1) 2) (bitand x 1))
	      (-fx y 2)))
...
;;; 2019-07-01: vector-for-each is now a native Bigloo procedure
