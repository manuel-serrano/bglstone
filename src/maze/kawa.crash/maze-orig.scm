  ...
  (let ((v (make-vector (*fx nrows ncols))))
	...
	((<fx r 0))
      (do ((c 0 (+fx c 1))
	   (i (*fx r ncols) (+fx i 1)))
	  ((=fx c ncols))
	(vector-set! v i (proc (*fx 3 c) (+fx (*fx 2 r) (logand c 1))))))
...
(define (make-wall-vec harr)
  (let* ((nrows (harr:nrows harr))
	...
	((<fx x 0))
      (do ((y (+fx (*fx (-fx nrows 1) 2) (logand x 1))
	      (-fx y 2)))
   ...
   (let* ((boxs (gen-maze-array nrows ncols))
	 ...
	 (lambda (entrance exit) 
	    (let* ((exit-box (href/rc boxs 0 exit))
	       ...
	       (mark-path exit-box)
	       (set-box:walls exit-box (logand walls (lognot south)))
	 ...
	 (let* ((c1   (wall:owner wall)) ; find the boxs on
	   ...
	   (if (not (set-equal? set1 set2))
	       (let ((walls (box:walls c1))	
		     (wall-mask (lognot (wall:bit wall))))
		 (union! set1 set2)
		 (set-box:walls c1 (logand walls wall-mask))
