...
(define (compiler-error msg . args)  
  ...
  (display "*** ERROR -- ")
  (display msg)
  (Sfor-each2 (lambda (x)   (display " ") (write x)) args)
  (newline)
  (compiler-abort))
...
(define (node-children-set! x y)  
  (Svector-set! x 2 y)
  (Sfor-each2 (lambda (child)   (node-parent-set! child x)) y)
...
(define (new-prc source decl name min rest parms body)  
  (let ((node (make-prc #f (list body) #t decl source name min rest parms)))
    (Sfor-each2 (lambda (x)   (var-bound-set! x node)) parms)
...
(define (new-call source decl oper args)  
  (let ((node (make-app #f (cons oper args) #t decl source)))
    ...
    (Sfor-each2 (lambda (x)   (node-parent-set! x node)) args)
    node))
(define (new-call* source decl oper args)  
  (if *ptree-port*
      (if (ref? oper)
...
(define (ll!-let ptree cst-procs env)  
  (let* ((proc (app-oper ptree))
    ...
    (let* ((tcfv-map
        ...
        (let* ((val (var->val var)) (vars (lifted-vars var)))
          (if (not (null? vars))
              (let ((var-copies (Smap2 var-copy vars)))
                ...
                (Sfor-each2 (lambda (x)   (var-bound-set! x val)) var-copies)
      ...
      (Sfor-each2 (lambda (node)   (ll! node cst-procs* env)) vals)
...
(define (ll-rename! ptree var-map)  
  ...
  (node-fv-set! ptree #t)
  (Sfor-each2 (lambda (child)   (ll-rename! child var-map)) (node-children ptree)))
    ...
    (if (or (not (null? decls)) (not (null? procs)) (not (null? inits)))
        (let* ((module-name (proc-obj-name module-init-proc))
          ...
          (Sfor-each2
           ...
           procs)
          (newline port)
          (Sfor-each2 (lambda (x)   (display x port) (newline port)) decls)
          (if (not (null? procs))
              (begin
                (Sfor-each2
                 (lambda (x)  
          ...
          (newline port)
          (Sfor-each2 (lambda (x)   (display x port) (newline port)) inits)
          (display c-id-prefix port)
            ...
            (let* ((calling-local-proc?
                    (and (ref? oper)
                         (let ((opnd (var->opnd (ref-var oper))))
                           (and (lbl? opnd)
                                (let ((x (Sassv (lbl-num opnd) known-procs)))
                                  (and x
                                       (let ((proc (Scdr x)))
...
(define (free-vars-order l)  
  (let ((bins '()) (ordered-args '()))
    ...
    (Sfor-each2
     (lambda (x)   (Sfor-each2 add-to-bin! (set->list (free-v (Scar x)))))
     l)
...
(define (ofile-comment l)  
  (if ofile-asm?
      (let ()
        ...
        (Sfor-each2 (lambda (x)   (asm-display x *ofile-port2*)) l)
        (asm-newline *ofile-port2*)
        (set! *ofile-pos* 0))))
...
(define (asm-assemble! debug-info)  
  ...
  (let ((code-list (queue->list asm-code-queue))
    ...
    (let* ((fix-list
          ...
          (if (not (null? l))
              (let ((part (Scar l)) (rest (Scdr l)))
                (if (pair? part)
                    (case (Scar part)
                      ((brab)
                       (Sset-cdr! (Scdr part) (Scdr (Sassv (Scddr part) lbl-list))))
                      ((wrel)
                       (Sset-car! (Scdr part) (Scdr (Sassv (Scadr part) lbl-list))))
                      ((lrel)
                       (Sset-car!
                        (Scdr part)
                        (Scdr (Sassv (Scadr part) lbl-list))))))
                (loop rest)))))
      ...
      (define (write-code)  
        (let ((proc-len
               (SFX+ (Scadr (Scdr (Sassv const-lbl lbl-list)))
...
(define (copy-opnd-to-loc opnd loc sn)  
  (if (and (lbl? opnd) (eqv? loc return-reg))
...
(define (commut-oper gen opnds loc sn self? accum-self accum-other)  
  (if (null? opnds)
      ...
      (let ((opnd (Scar opnds)) (rest (Scdr opnds)))
        (cond ((and (not self?) (eqv? opnd loc))
...
(define (gen-sub first-opnd other-opnds loc sn self-opnds?)  
  (if (null? other-opnds)
      (if (and (or (reg? loc) (stk? loc)) (not (eqv? loc return-reg)))
          (begin
...
(define (make-gen-vector-length kind)  
  (lambda (opnds loc sn)  
    (let ((sn-loc (sn-opnd loc sn))
          (opnd (Scar opnds))
          (reg68 (if (and (reg? loc) (not (eqv? loc return-reg)))
                     (reg->reg68 loc)
                     dtemp1)))
      (move-opnd-to-loc68 opnd atemp1 sn-loc)
