...
class almabench
{
    ...
    private static final double JCENTURY  = 36525.0;
    private static final double JMILLENIA = 365250.0;
    private static final double TWOPI     = 2.0 * PI;
    private static final double A2R       = PI / 648000.0;
    private static final double R2H       = 12.0 / PI;
    ...
    private static final int TEST_LOOPS  = 4;
    private static final int TEST_LENGTH = 36525;

    // sin and cos of j2000 mean obliquity (iau 1976)
    private static final double sineps = 0.3977771559319137;
    private static final double coseps = 0.9174820620691818;

    private static final double [] amas = { 6023600.0, 408523.5, 328900.5, 3098710.0, 1047.355, 3498.5, 22869.0, 19314.0 };
    ...
    private static final double [][] dlm =
    ...
    private static final double [][] dinc =
    ...
    private static final double [][] omega =
    ...
    private static final double [][] ca =
    ...
    private static final double [][] cl =
    ...
    private static double anpm (double a)
    {
        double w = a % TWOPI;
    ...
    private static void planetpv (double [] epoch, int np, double [][] pv)
    {
        // working storage
        int        k;
        ...
        for (k = 0; k < 8; ++k)
        {
            arga = kp[np][k] * dmu;
            argl = kq[np][k] * dmu;
            da   += (ca[np][k] * Math.cos(arga) + sa[np][k] * Math.sin(arga)) * 0.0000001;
            dl   += (cl[np][k] * Math.cos(argl) + sl[np][k] * Math.sin(argl)) * 0.0000001;
        }

        arga = kp[np][8] * dmu;
        da   += t * (ca[np][8] * Math.cos(arga) + sa[np][8] * Math.sin(arga)) * 0.0000001;

        for (k = 8; k <= 9; ++k)
        {
            argl = kq[np][k] * dmu;
            dl   += t * ( cl[np][k] * Math.cos(argl) + sl[np][k] * Math.sin(argl) ) * 0.0000001;
        }
        ...
        v = GAUSSK * Math.sqrt((1.0 + 1.0 / amas[np] ) / (da * da * da));
    ...
    // planetpv.
    private static void radecdist(double [][] state, double [] rdd)
    {
        // distance
        rdd[2] = Math.sqrt(state[0][0] * state[0][0] + state[0][1] * state[0][1] + state[0][2] * state[0][2]);

        // RA
        rdd[0] = Math.atan2(state[0][1], state[0][0]) * R2H;
        if (rdd[0] < 0.0)
          rdd[0] += 24.0;

        // Declination
        rdd[1] = Math.asin(state[0][2] / rdd[2]) * R2D;
    }
        ...
        for (int i = 0; i < TEST_LOOPS; ++i)
        {
            ...
            for (int n = 0; n < TEST_LENGTH; ++n)
            {
                ...
                for (int p = 0; p < 8; ++p)
                {
                    ...
                    // System.out.println(p + " " + position[0] + " " + position[1]);
                }
            }
        }
