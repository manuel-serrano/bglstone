...
let print_vector v =
  print_string "#(";
  iter (fun x -> print_float x; print_string " ") v;
  print_string ")"
...
let j2000       = 2451545.0
let jcentury    = 36525.0
let jmillenia   = 365250.0
let twopi       = 2.0 *. pic
let a2r         = pic /. 648000.0
let r2h         = 12.0 /. pic
let r2d         = 180.0 /. pic
let gaussk      = 0.01720209895
...
let sineps      = 0.3977771559319137
let coseps      = 0.9174820620691818

let amas        = [|  6023600.0; 408523.5; 328900.5; 3098710.0; 1047.355; 3498.5; 22869.0; 19314.0 |]
...
let dlm = [| [| 252.25090552;  5381016286.88982;   -1.92789 |];
...
let e = [| [| 0.2056317526;   0.0002040653;     -28349e-10 |];
...
let pi = [| [|  77.45611904;   5719.11590;    -4.83016 |];
...
let dinc = [| [|  7.00498625;  -214.25629;    0.28977 |];
...
let omega = [| [|  48.33089304;   -4515.21727;   -31.79892 |];
...
let kp = [| [| 69613.0;  75645.0; 88306.0; 59899.0; 15746.0; 71087.0; 142173.0;  3086.0;    0.0 |];
...
let ca = [| [|       4.0;    -13.0;    11.0;    -9.0;    -9.0;    -3.0;    -1.0;     4.0;    0.0 |];
...
let sa = [| [|     -29.0;    -1.0;     9.0;     6.0;    -6.0;     5.0;     4.0;     0.0;    0.0 |];
...
(* tables giving the trigonometric terms to be added to the mean elements of
   the mean longitudes . *)
let kq = [| [|  3086.0; 15746.0; 69613.0; 59899.0; 75645.0; 88306.0; 12661.0; 2658.0;  0.0;   0.0 |];
...
let cl = [| [|      21.0;   -95.0; -157.0;   41.0;   -5.0;   42.0;   23.0;   30.0;     0.0;    0.0 |];
...
let sl = [| [|   -342.0;   136.0;  -23.0;   62.0;   66.0;  -52.0;  -33.0;   17.0;     0.0;    0.0 |];
