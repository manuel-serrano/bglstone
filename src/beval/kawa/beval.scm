;*=====================================================================*/
;*    serrano/diffusion/article/bjvm/bench/beval/kawa/beval.scm        */
;*    -------------------------------------------------------------    */
;*    Author      :  Manuel Serrano                                    */
;*    Creation    :  Thu Jun  1 11:05:12 1995                          */
;*    Last change :  Wed Feb 14 09:29:15 2001 (serrano)                */
;*    -------------------------------------------------------------    */
...
(define (compile-global-application proc actuals tail?)
   (case (length actuals)
      ((0 1 2 3)
       `(,(+ (length actuals) 27) ,proc ,@actuals))
      (else
       `(32 ,proc ,@actuals))))
...
(define (compile-compiled-application proc actuals tail?)
   (case (length actuals)
      ((0 1 2 3)
       `(,(+ (length actuals) 33) ,proc ,@actuals))
      (else
       `(38 ,proc ,@actuals))))
...
(define (compile-application proc actuals tail?)
   (case (length actuals)
      ((0 1 2 3)
       `(,(+ (length actuals) 39) ,proc ,@actuals))
      (else
       `(44 ,proc ,@actuals))))

(linit-the-global-environment!)
(ldefine-primitive! '+ (lambda (x y) (+ x y)))
(ldefine-primitive! '- (lambda (x y) (- x y)))
(ldefine-primitive! '< (lambda (x y) (< x y)))
