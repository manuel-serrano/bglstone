...
(define (lsort obj pred)
  ...
  (cond ((or (pair? obj) (null? obj))
	 (loop obj))
	((vector? obj)
	 (lsort! (vcopy obj) pred))
	(else
	 (error '() "lsort: argument should be a list or vector" obj))))

;; This merge lsort is stable for partial orders (for predicates like
;; <=, rather than like <).

(define (lsort! v pred)
  (define (lsort-internal! vec temp low high)
    (if (< low high)
	(let* ((middle (quotient (+ low high) 2))
	       (next (+ middle 1)))
	  (lsort-internal! temp vec low middle)
	  (lsort-internal! temp vec next high)
  ...
  (if (not (vector? v))
      (error '() "lsort!: argument not a vector" v))
...
(define expected-result
   '("(((b v d) ^ a) v c)" "(c ^ d)" "(b v (a ^ d))" "((a v d) ^ b)" "(b v d)" "(b ^ (a v c))" "(a v (c ^ d))" "((b v d) ^ a)" "(c v (a v d))" "(a v c)" "(d v (b ^ (a v c)))" "(d ^ (a v c))" "((a ^ d) v c)" "((a ^ b) v d)" "(((a v d) ^ b) v (a ^ d))" "(b ^ d)" "(b v (a v d))" "(a ^ c)" "(b ^ (c v d))" "(a ^ b)" "(a v b)" "((a ^ d) ^ b)" "(a ^ d)" "(a v d)" "d" "(c v d)" "a" "b" "c" "any" "none"))
...
(define (main argv)
   (let loop ((i (if (pair? (cdr argv))
		     ...
		     4500)))
      (if (= i 1)
	  (exit (if (equal? (test) expected-result) 0 1))
	  (begin
	     (test)
	     (loop (- i 1))))))
