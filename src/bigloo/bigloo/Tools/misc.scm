...
;*    Last change :  Mon Nov 29 13:17:09 2004 (serrano)                */
...
(module tools_misc
   (export (string*->string::bstring ::pair-nil)
	   (replace! p1 p2)
	   (string-split::pair-nil str::bstring separator::bchar)
	  ...
	  (let loop ((i 0))
	     (cond ((>= i str-length)
		   ...
		   ((char=? (string-ref str i) separator)
		    (if (zero? i)
			(string-split (substring str 1 str-length) separator)
			(cons (substring str 0 i)
			      (string-split (substring str (+ i 1) str-length)
					    separator))))
		   (else
		    (loop (+ i 1))))))))
