...
(define (java-parser java module)
   ...
   (match-case java
      ...
      ((export (and (? symbol?) ?bname) (and (? string?) ?cname))
       (set! *jexported* (cons java *jexported*)))
      ((export . ?-)
...
(define (java-finalizer)
   ...
   (for-each (lambda (java)
		(let ((global (find-global (cadr java)))
		      (name   (caddr java)))
		   (cond
		      ((not (global? global))
...
(define (java-declare-component j jklass::jklass component)
   ...
   (match-case component
	  ...
	  (let ((jconstr (instantiate::jconstructor
			    ...
			    (args (if *saw*
				      args
				      (cons (make-typed-formal idd) args)))
			    (jname "<init>"))))
...
(define (declare-jklass jklass::jklass)
   (with-access::jklass jklass (id jname constructors methods src loc)
	 ...
	 (with-access::jmethod jmet (id args jname src modifiers)
	    (let ((g (declare-java-method! id jname args modifiers
					   (jklass-jname jklass)
					   src)))
	       (global-module-set! g (jklass-id jklass))
	       g)))
...
;*    declare-java-method! ...                                         */
;*---------------------------------------------------------------------*/
(define (declare-java-method! id jname args modifiers kname src)
   (let* ((pid (parse-id id (find-location src)))
	  (ln (car pid))
	  (tid (type-id (cdr pid))))
      (let ((g (declare-global-cfun! ln jname tid args #f #f src #f)))
