...
;*    Last change :  Tue Oct 19 16:20:38 2004 (serrano)                */
...
(define (dotnet-ld)
   (let* ((target (if (string? *dest*)
	  ...
	  (bigloo-lib (make-bigloo-dll))
	  (other-libs (map (lambda (lib)
			      (lib->string
			       (user-lib-name lib) (dotnet-library-suffix)))
...
(define (dotnet-libdir)
   (if (string? *dotnet-dll-path*)
       *dotnet-dll-path*
       bgl-configure-dll-directory))

;*---------------------------------------------------------------------*/
;*    make-bigloo-dll ...                                              */
;*---------------------------------------------------------------------*/
(define (make-bigloo-dll)
   (lib->string (user-lib-name *bigloo-vlib*) (dotnet-library-suffix)))
