	 ...
	 (let ( (params (map local->reg! args)) )
	    ...
	    (let ( (locals (get-locals params l)) )
		  ...
		  (if (and *c-debug-lines-info*
	       ...
	       (display " {AN_OBJECT\n")
	       (if *hasprotect* (display " jmp_buf jmpbuf;\n"))
...
(define (print-location? loc)
   (if (location? loc)
       (begin (print "# " (location-lnum loc) " \"" (location-fname loc) "\"")
	      #t )
       #f ))
...
(define (basic-type? type::type) ;()
   (with-access::type type (id)
      (memq id '(double llong elong long int uchar bool string)) ))
...
;;
(define-method (backend-subtype? b::cvm t1::type t2::type)
